...
Signal.link = curry((fn, inputs) => {
  ...
  return evaluate(Signal(self))
})

const unlink = (signal, dependent, cb = () => {}) => {
...
Signal.chain = curry((fn, sa) => {
  ...
  const self = Signal.of()
  Signal.link(s => {
    if (signal && effect) unlink(signal, effect, signal.dispose)
...
const set = curry((signal, value) => {
  if (value === undefined) {
    ...
    signal.value = value
    signal.dirty = true
    // Topological order (leafs last).
    // Note: sort must be stable to keep order of same levels.
    dependent(signal).sort(compare).forEach(evaluate)
  }
 ...
 */
const dependent = (signal, acc = [], level = 1) => {
  return signal.dependent.reduce((acc, x) => {
    if (!x.level) { x.level = level; acc.push(x) }
    return dependent(x, acc, level + 1)
  }, acc)
}
